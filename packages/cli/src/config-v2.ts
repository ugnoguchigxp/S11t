import type { CanonicalVariableDefinitionV2 } from "@s11t/runtime/compiler";

import { S11tDiagnosticError, type S11tDiagnostic } from "./diagnostics.js";

export type S11tReleaseProfileV2 = {
...
export type S11tProjectConfigV2 = {
	...
	releaseProfiles: Record<string, S11tReleaseProfileV2>;
	variableProfiles: Record<string, CanonicalVariableDefinitionV2>;
...
function parseVariableProfiles(
	...
	const result: Record<string, CanonicalVariableDefinitionV2> = {};
	for (const [name, value] of Object.entries(source)) {
		...
		result[name] = {
			required: true,
			type: type as CanonicalVariableDefinitionV2["type"],
			trust: trust as CanonicalVariableDefinitionV2["trust"],
			placement: placement as CanonicalVariableDefinitionV2["placement"],
			encoding: encoding as CanonicalVariableDefinitionV2["encoding"],
		};
	}
	return result;
}

export function parseProjectConfigV2(
	input: unknown,
	...
	const releaseProfiles: Record<string, S11tReleaseProfileV2> = {};
	...
	for (const [alias, value] of Object.entries(aliasSource)) {
		if (!ALIAS_KEY_PATTERN.test(alias)) issue(file, "Invalid alias key", ["key_aliases", alias]);
