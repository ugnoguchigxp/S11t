...
consumer testは少なくともNode 20.19と24で実行する。package tarball作成はNode 20.19 / 24 / Ubuntuで実行し、consumer smokeは
...
```text
verify matrix
  Node 20.19 / 22 / 24 × Linux / macOS / Windows

package job
  Ubuntu + Node 20.19 / 24
  build
  pack:all
...
| Cross platform | GitHub Actions | Node 20.19/22/24、Linux/macOS/Windowsでconsumer成功 |
...
- CIにpackage / consumer Gateがあり、Node 20.19/22/24と3 OSの必要な証拠が揃う。
